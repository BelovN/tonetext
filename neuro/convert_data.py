...
from settings import DATA_DIR, GOOGLE_VECTORS

nltk.download('punkt')



def load_data(path=DATA_DIR, count=100):
    data = pd.read_csv(path).iloc[:count]
    classes = []
    ...
    data = data.assign(classes=classes)
...
def get_data(count=100):
    data = load_data(count=count)
    return data['review'], data['classes']


def get_standartized_data(count=100):
    data_x, data_y = get_data(count=count)
...
def fit_bow_vectorizer(data):
    count_vectorizer = CountVectorizer()
    emb = count_vectorizer.fit_transform(data)
    return emb, count_vectorizer


def get_bow_convertation_data(count=100):
    data_x, data_y = get_data(count=count)
    data_x = standartize_data(data_x)
    train_x, test_x, train_y, test_y = train_test_split(data_x, data_y, test_size=0.2, random_state=40)
    train_x, count_vectorizer = fit_bow_vectorizer(train_x)
...
def get_tf_idf_convertation_data(count=100):
    data_x, data_y = get_data(count=count)
    data_x = standartize_data(data_x)
    train_x, test_x, train_y, test_y = train_test_split(data_x, data_y, test_size=0.2, random_state=40)
...
def get_w2v_convertation_data(generate_missing=False, count=100):
    w2v_path = GOOGLE_VECTORS
